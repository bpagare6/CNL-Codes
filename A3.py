#
# Write a program in Java/Python to demonstrate sub netting and find the sub net
# masks.
#
...
# Get the Class of IP and Default subnet mask
...
# PING(Packet InterNet Groper) to given IP address
...
if __name__ == "__main__":
    ...
    # Check the class of IP address is A, B or C.
    if ip_class in 'ABC':
        print('\nClass of', ip_address, 'is', ip_class,
              '& Default Subnet mask is', default_subnet)
        ...
        subnet_mask = default_subnet.replace("0", str(last_subnet_value), 1)
        print('\nNew Subnet mask is', subnet_mask)

        # Uncomment this line if you want to change IP address of your machine by program
        ...
        # Find the IP range that you belong
        if ip_class == 'A':
            splitted_ip = str(ip_address).split('.')
            ip_start = splitted_ip[0]
            last_ip_part = int(str(ip_address).split('.')[-3])
        elif ip_class == 'B':
            splitted_ip = str(ip_address).split('.')
            ip_start = splitted_ip[0] + '.' + splitted_ip[1]
            last_ip_part = int(str(ip_address).split('.')[-2])
        else:
            splitted_ip = str(ip_address).split('.')
        ...
        for i in range(number_of_subnets):
            ...
            if last_ip_part >= start_ip_value and last_ip_part <= last_ip_value:
                ...
                if ip_class == 'A':
                    start_ip_range += '.255.255'
                    last_ip_range += '.255.255'
                elif ip_class == 'B':
                    start_ip_range += '.255'
                    last_ip_range += '.255'
