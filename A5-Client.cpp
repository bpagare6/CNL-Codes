 ...
 * Client Code
 *
 * g++ A5-Client.cpp -o Client
 * ./Client <IP address of server> <Destination folder>
 */
...
int main(int argc, char **argv) {
  ...
  if (argc != 3) {
    cout << "Usage: " << argv[0]
         << " <IP address of server> <Destination folder>\n";
    return 1;
  }
  ...
  cin >> buffer;
  sendto(sock_fd, buffer, BUFFER_SIZE, 0, (struct sockaddr *)&server_address,
         address_length);
  ...
  number_of_bytes =
      recvfrom(sock_fd, (char *)&file_size, sizeof(file_size), 0,
  ...
  if (fp > 0) {
    while (number_of_bytes <= file_size + 1) {
      ...
      // cout << "\n" << number_of_bytes << " of bytes received from IP " <<
