/*
 * Write a program in C/C++/Java/Python to analyze following packet formats
 * captured through Wireshark for wired network. 1. TCP 2. IP 3. UDP
 *
 * Author: Bhushan Pagare
 *
 * g++ A7.cpp -o A7 -lpcap
 * ./A7 <Name of PCAP file>
 *
 * Install pcap.h: su, yum install libpcap-devel
 */
...
int main(int argc, char **argv) {
  ...
  cout << "IP address that most HTTP/HTTPS traffic goes to (in terms of "
          "bandwidth, NOT packet count): "
       << httpIP[maxIdxHttp] << "\n";
  return 0;
}
  ...
  if (ntohs(ethernetHeader->ether_type) == ETHERTYPE_IP) {
    ...
    if (ipHeader->ip_p == IPPROTO_TCP) {
      ...
      dataLength = pkthdr->len - (sizeof(struct ether_header) +
                                  sizeof(struct ip) + sizeof(struct tcphdr));
      if (sourcePort == 80 || sourcePort == 443 || destPort == 80 ||
          destPort == 443) {
        for (i = 0; i < httpIdx; i++) {
